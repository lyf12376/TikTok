                        ...
                        trailingIcon = {
                            Icon(
                                ...
                                modifier = Modifier.clickable {
                                    scope1.launch {
                                        loginViewModel.deleteSavedUser(savedUser.account)
                                    }
                                }
                            )
                        }
            ...
            onClick = {
                ...
                runBlocking {
                    withContext(Dispatchers.IO){
                        val isLogin = loginViewModel.isLogin(account, password = password)
                        if (isLogin) {
                            navigate = true
                            if (rememberPassword) {
                                val savedUser = SavedUser(account = account, password = password)
                                loginViewModel.saveUsers(savedUser)
                            }
                        } else {
                            showDialog = true
                        }
                    ...
                    if (navigate) {
                        scope1.launch {
                            loginViewModel.saveUsers(SavedUser(account = account, password = password))
                        }
